...
plotdata <- plotdata %>%
  ...
  mutate(vulnerability = case_when(str_detect(outcome, '_no') ~ 'No vulnerability', T ~ 'High vulnerability'),
...
plot_1 <- plotdata %>% 
  ...
  geom_point() + geom_line() + xlab('vulnerability scale (left), resilience scale (right)')+
...
plot_2 <- plotdata %>% 
  ...
  geom_point() + geom_line() + ylab('Odds (log)') + xlab('vulnerability scale (left), resilience scale (right)')+
  facet_wrap(~vulnerability) + theme_bw(base_size = 14) + 
...
for(region in c('', 'Tunisia', 'Chile')){
  ...
  for(outcome in vul_outcomes){
    ...
    if(region == ''){
      ...
      # hierarchical boosting for interactions
